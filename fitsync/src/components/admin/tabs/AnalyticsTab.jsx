  ...
  const getResponseTimeDistribution = () => {
    ...
    const distribution = ranges.map(range => {
      const count = filteredLogs.filter(log => {
        const time = parseFloat(log.apilog_response_time);
    ...
    return {
      ...
      datasets: [{
        ...
        data: distribution,
        backgroundColor: [
          '#10b981', '#3b82f6', '#f59e0b', '#f97316', '#ef4444', '#991b1b'
        ],
        borderWidth: 1
      }]
    };
              ...
              <AnalysisGrid>
                ...
                <InsightsPanel>
                  ...
                  {responseTimeData && (
                    <PerformanceStats>
                      <StatRow>
                        <StatLabel>고속 응답 (&lt; 300ms)</StatLabel>
                        <StatValue good>
                          {(((responseTimeData.datasets[0].data[0] + responseTimeData.datasets[0].data[1]) / 
                              responseTimeData.datasets[0].data.reduce((a, b) => a + b, 0)) * 100).toFixed(1)}%
                        </StatValue>
                      </StatRow>
                      <StatRow>
                        <StatLabel>느린 응답 (&gt; 1s)</StatLabel>
                        <StatValue warning>
                          {(((responseTimeData.datasets[0].data[4] + responseTimeData.datasets[0].data[5]) / 
                              responseTimeData.datasets[0].data.reduce((a, b) => a + b, 0)) * 100).toFixed(1)}%
                        </StatValue>
                      </StatRow>
                    </PerformanceStats>
